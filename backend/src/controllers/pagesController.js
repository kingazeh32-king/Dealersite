...
const PAGE_FIELDS = db.pages.FIELDS;

function buildDefaultPage(slug) {
  const defaults = {
    'about': {
      title: 'About Us',
      subtitle: 'A family-focused dealership helping buyers find quality manufactured and tiny homes.',
      sections: [],
      highlights: [],
    },
    financing: {
...
async function list(req, res, next) {
  try {
    const rows = await db.pages.findAll();
    res.json({ rows });
  } catch (err) {
    next(err);
  }
...
async function getBySlug(req, res, next) {
  try {
    let page = await db.pages.findBySlug(req.params.slug);

    if (!page) {
      const defaults = buildDefaultPage(req.params.slug);
      page = await db.pages.create(req.params.slug, defaults);
    }
...
async function update(req, res, next) {
  try {
    ...
    let page = await db.pages.update(slug, data);

    if (!page) {
      const defaults = buildDefaultPage(slug);
      page = await db.pages.create(slug, { ...defaults, ...data });
    }
